...
const testimonials = [
  {
    quote:
      "We ship 200+ Amazon orders daily. LabelCrop cut our label prep from 45 minutes to under 5. The invoice text extraction alone saves our packers hours every week.",
    name: "Priya Sharma",
  ...
  return (
    <section id="testimonials" className="scroll-mt-20 py-20 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          eyebrow="Customers"
          title="Loved by sellers who ship at scale"
          description={`eCommerce teams worldwide trust ${SITE.name} for secure, watermark-free label cropping.`}
        />
        ...
        <div className="mt-14 grid gap-4 lg:grid-cols-3">
          {testimonials.map((item) => (
            <article
              key={item.name}
              className="card-hover flex flex-col rounded-[var(--radius-card)] border border-border bg-card p-6"
            >
              <Quote className="h-7 w-7 text-primary/20" strokeWidth={1.5} />
              ...
              <footer className="mt-6 border-t border-border pt-4">
                ...
                <span className="mt-3 inline-block rounded-full border border-primary/15 bg-primary/8 px-2.5 py-0.5 text-[11px] font-medium text-primary">
                  {item.platform}
                </span>
              </footer>
            </article>
